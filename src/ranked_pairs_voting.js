...
function handleFileSelect(event){
  ...
  fileReader.onload = function(event){
    ...
    let fileContents = fileReader.result || event.target.result;
...
//store the data
let seperators = ["Timestamp", "Name", "Email Address", "Do you approve the draft bylaws and petition for autonomous status?", []];
let ballots = [];
...
function draw(){
  ...
  if(diplayContent && fileContent !== ""){
    ...
    let resultsTable = document.getElementById("fileContent");
    resultsTable.innerHTML = "";
    
    let temp = fileContent.split("\n");
    let topRow = temp[0].split(",");
    //add seperators
    ...
    for(let i = 0; i < seperators.length; i++){
      if(typeof seperators[i] === "string"){
        ...
        for(let j = 0; j < seperators[i].length; j++){
    ...
    for(let i = 0; i < seperators[seperators.length-1].length; i++){
    ...
    //find all the pairs
    let pairsOut = document.getElementById("pairs");
    let candidates = seperators[seperators.length-1];
    let pairs = [];
    ...
    for(let p = 0; p < pairs.length; p++){
      ...
      let candidate1Index = candidates.indexOf(pairs[p][0]) + seperators.length-1;
      let candidate2Index = candidates.indexOf(pairs[p][1]) + seperators.length-1;
      
      //loop through all the ballots to find the scores
      for(let b = 0; b < ballots.length; b++){
        //get the candidate content for the ballot
        let ballotContent1 = ballots[b][candidate1Index].toLowerCase();
        let ballotContent2 = ballots[b][candidate2Index].toLowerCase();
